    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.content}>
        <ThemedView style={styles.section}>
          <ThemedText style={styles.sectionTitle}>Generate Schedule</ThemedText>
          <ThemedView style={styles.inputRow}>
            <ThemedView style={styles.inputContainer}>
              <ThemedText style={styles.inputLabel}>Number of places</ThemedText>
              <TextInput
                style={styles.rowInput}
                placeholder="Enter number"
                keyboardType="numeric"
                value={numLocations}
                onChangeText={setNumLocations}
              />
            </ThemedView>
            <ThemedView style={styles.inputContainer}>
              <ThemedText style={styles.inputLabel}>Target area</ThemedText>
              <TextInput
                style={styles.rowInput}
            ...
            <ThemedView style={styles.inputContainer}>
              <ThemedText style={styles.inputLabel}>Maximum radius</ThemedText>
              <TextInput
                style={styles.rowInput}
          ...
          >
            {ACTIVITY_TYPES.map((type) => (
              <TouchableOpacity
                ...
                style={[
                  styles.activityChip,
                  activityType.has(type) && styles.activityChipSelected,
                ]}
                onPress={() => setActivityType(prev => {
              ...
              >
                <ThemedText
                  style={[
                    ...
                    activityType.has(type) && styles.activityChipTextSelected,
                  ]}
                >
                  {type}
                </ThemedText>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              // Get all locations from selected folders
              const selectedFolderLocations = mockFolders
                .filter(folder => selectedFolders.has(folder.title))
                .flatMap(folder => folder.locations);
              
              // Randomly select the specified number of locations
              const numToSelect = Math.min(
                parseInt(numLocations) || 1,
                selectedFolderLocations.length
              );
              
              const shuffled = [...selectedFolderLocations]
                .sort(() => Math.random() - 0.5)
                .slice(0, numToSelect);
              
              setSelectedLocations(shuffled);
            }}
          >
            <ThemedText style={styles.generateButtonText}>Generate Itinerary</ThemedText>
          </TouchableOpacity>
        </ThemedView>
